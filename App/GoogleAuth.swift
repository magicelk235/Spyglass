        ...
        let port: UInt16 = try await withCheckedThrowingContinuation { portCont in
            ...
            listener.newConnectionHandler = { [weak box] conn in
                ...
                conn.receive(minimumIncompleteLength: 1, maximumLength: 4096) { data, _, _, _ in
                    ...
                    let body = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Spyglass</title><style>:root{color-scheme:light dark}html,body{height:100%;margin:0}body{display:flex;align-items:center;justify-content:center;font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#f5f5f7;color:#1d1d1f}@media(prefers-color-scheme:dark){body{background:#1d1d1f;color:#f5f5f7}}.card{text-align:center;padding:5vh 6vw}.check{width:clamp(72px,10vw,140px);height:clamp(72px,10vw,140px);margin:0 auto clamp(24px,4vh,48px);border-radius:50%;background:#B8945F;display:flex;align-items:center;justify-content:center}.check svg{width:55%;height:55%}.mark{font-size:clamp(16px,1.6vw,24px);font-weight:600;letter-spacing:.06em;color:#B8945F;margin:0 0 clamp(12px,2vh,24px)}h1{font-size:clamp(40px,6vw,88px);font-weight:700;margin:0 0 clamp(12px,2vh,20px);line-height:1.05}p{font-size:clamp(18px,2vw,30px);opacity:.55;margin:0}</style></head><body><div class=\"card\"><div class=\"check\"><svg viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"#fff\" stroke-width=\"3\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M5 13l4 4L19 7\"/></svg></div><p class=\"mark\">SPYGLASS</p><h1>You're signed in</h1><p>You can close this tab and return to the app.</p></div></body></html>"
